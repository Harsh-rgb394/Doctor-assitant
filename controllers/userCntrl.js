...
const Logincontroller=async(req,res)=>{
    try {
        ...
        const comapredpassword=await bcrypt.compare(req.body.password,existuse.password);
        if(!comapredpassword){
            res.status(200).send({message:"password doesnot match ",success:false});
        }

        // after successfulyy login wiht email and password we generate token 
        const token=jsonwebtoken.sign({id:existuse._id},process.env.JWT_SECRET,{
            expiresIn:"1d" 
        })
        res.status(200).send({message:"user login successfully",success:true,token});
...
const Registercontroller= async(req,res)=>{
    try {
       ...
       if(existuser){
        res.status(201).send({message:"user already regsiter",success:false});
       }
...
module.exports={Logincontroller,Registercontroller,authController,applyDoctorController,getallnotify,deleteallnotify,getlistdoctorscontroller,bookappointmentcontroller,getallappointmentcontroller,bookappointavailablecontroller};